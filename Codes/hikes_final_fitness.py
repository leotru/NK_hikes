...
# Project NK-hikes/
# ------------------------------------------------------------------------
# hikes_final_fitness.py / version 0.1 
...
# ------------------------------------------------------------------------
# Visualize the histogram of fitness maxima per hike
f = np.array(fitness)

#plt.title(f"{title}", fontsize = 14)
plt.title(r"$N$" f" = {N}," r" $K$" f" = {K}," r" $epi$" f" = {epi}," r" $p$" f" = {M}," r" hikes" f" = {realizations} ", fontsize = 14)

plt.hist(f[:,1],bins = [0.60 + k * 0.005 for k in range(29)])

plt.xlabel('final fitness values by hike',fontsize = 14)
plt.xlim([0.6, 0.74])
plt.grid(which = 'minor',axis='both',  alpha = 0.3)
plt.grid(which = 'major', axis='both', alpha = 0.7)  
 

fig_file_name = ("figFinalFitness_N%s_K%s_T%s_epi%s_M%s_hikes%s.png" % (N, K, T, epi, M, realizations))
